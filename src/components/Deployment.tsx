    ...
    <section className="py-28 px-6 bg-dark-50">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-20">
          <p className="text-primary-600 font-medium text-sm tracking-widest uppercase mb-4">Deployment</p>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {options.map((opt) => (
            <div
              key={opt.title}
              className="bg-white rounded-2xl p-8 text-center hover:shadow-lg transition-all duration-300 group"
            >
              <div className="w-14 h-14 rounded-full bg-dark-50 text-dark-500 group-hover:text-primary-600 flex items-center justify-center mx-auto mb-6 transition-colors duration-300">
                {opt.icon}
              </div>
              <h3 className="text-lg font-semibold text-dark-800 mb-3 tracking-tight">{opt.title}</h3>
              <p className="text-dark-400 text-sm leading-relaxed mb-5 font-light">{opt.description}</p>
              <span className="inline-block text-xs font-mono bg-dark-50 text-dark-500 px-4 py-1.5 rounded-full">
                {opt.detail}
              </span>
            </div>
          ))}
